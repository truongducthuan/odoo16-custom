...
class BearerToken(models.Model):
    ...
    def _check_token(self, token):
        if not token:
            return False
        self.env.cr.execute("""
            SELECT id, access_token, expiration_date FROM {table} 
            WHERE access_index = %s
        """.format(table=self._table), [token[:common.TOKEN_INDEX]])
        current_datetime = datetime.datetime.now()
        for token_id, token_hash, expiration_date in self.env.cr.fetchall():
            if expiration_date and current_datetime > expiration_date:
                # Token is expired
                return False

            if common.KEY_CRYPT_CONTEXT.verify(token, token_hash):
                # self.browse([token_expiration_date])
